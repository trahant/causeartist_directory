...
export const AdDeleteDialog = ({
  ...
  const ids = useMemo(() => ads.map(({ id }) => id), [ads])
  const pluralizedLabel = useMemo(() => plur("ad", ids.length), [ids])

  const deleteMutation = useMutation(
    orpc.ads.remove.mutationOptions({
      onSuccess: () => {
        toast.success("Ad(s) deleted successfully")
