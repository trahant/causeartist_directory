...
import { SubcategoryTableToolbarActions } from "~/app/admin/focus-areas/_components/subcategory-table-toolbar-actions"
...
const columns: ColumnDef<SubcategoryListRow>[] = [
  {
    id: "select",
    enableSorting: false,
    enableHiding: false,
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <DataTableLink href={`/admin/focus-areas/${row.original.id}`} title={row.original.name} />
    ),
  },
  {
    accessorKey: "slug",
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <Button variant="ghost" size="sm" className="size-8 p-0" asChild>
        <Link href={`/companies/focus/${row.original.slug}`} target="_blank" rel="noreferrer">
...
export function SubcategoryTable() {
  ...
  const { data, isLoading, isFetching } = useQuery(
    orpc.admin.subcategories.list.queryOptions({ input: params, placeholderData: keepPreviousData }),
  )

  const filterFields: DataTableFilterField<SubcategoryListRow>[] = [
  ...
  return (
    <DataTable table={table} isLoading={isLoading} isFetching={isFetching && !isLoading}>
      <DataTableHeader
        title="Focus areas"
        total={data?.subcategoriesTotal}
        callToAction={
          <Button variant="primary" size="md" prefix={<PlusIcon />} asChild>
            <Link href="/admin/focus-areas/new">New focus area</Link>
          </Button>
        }
      >
        <DataTableToolbar
