...
const getData = cache(async ({ params }: Props) => {
  ...
  const url = `/companies/focus/${subcategory.slug}`
  const title = `${subcategory.name} Companies | Causeartist`
  const description = `Discover mission-driven companies focused on ${subcategory.name} in the impact economy.`
  ...
  return { subcategory, companies, ...data }
})
...
export default async function CompaniesFocusPage(props: Props) {
  const { subcategory, companies, breadcrumbs, structuredData } = await getData(props)
