...
const getData = cache(async ({ params }: Props) => {
  ...
  const episode = await findPodcastEpisode({ where: { slug, show: "iip" } })

  if (!episode) {
    notFound()
  }

  const url = `/podcast/investing-in-impact/${episode.slug}`
  ...
  const data = getPageData(url, title, description, {
    breadcrumbs: [
      ...
      { url: "/podcast/investing-in-impact", title: "Investing in Impact" },
...
export const generateStaticParams = async () => {
  const episodes = await findPodcastEpisodeSlugs({ where: { show: "iip" } })
