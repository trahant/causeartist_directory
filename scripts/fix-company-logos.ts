...
async function main() {
  ...
  console.log(`Company logo candidates: ${candidates.length}`)

  for (const [idx, company] of candidates.entries()) {
    try {
      const website = company.website
      if (!website) {
        skipped++
        console.log(`[${idx + 1}/${candidates.length}] Skipping (no website): ${company.id}`)
        continue
      }
      ...
      if (company.logoUrl === faviconUrl) {
        skipped++
        console.log(`[${idx + 1}/${candidates.length}] Already correct: ${company.id}`)
        continue
      }
      ...
      updated++
      console.log(`[${idx + 1}/${candidates.length}] Updated: ${company.id}`)

      // Small delay to avoid hammering sites
      await delay(250)
    } catch (e) {
      failed++
      console.error(`[${idx + 1}/${candidates.length}] Failed: ${company.id}`, e)
    }
  }
